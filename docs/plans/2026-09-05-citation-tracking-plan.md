...
**Behaviour:**
- `find_superscript_citation_runs(skip_fields=True)` returns runs as before but (a) omits runs for which `self.fields.in_field(run)` when `skip_fields`, and (b) its `numbers` list is `expand_bracket_numbers(run.text, lenient=True)` so `3-5` → `[3,4,5]`; lenient means a token that is neither a number nor a well-formed range contributes the numbers written in it, so a run Word split at an rsid boundary (`3-` + `5`) is reported as `[3]` / `[5]` instead of being dropped (a dropped 3 would be renumbered as `3-12` and later seeded as uncited).
...
- `apply_renumbering` and `convert_in_text_to_author_date` skip in-field runs and skip bracket matches inside result spans. Superscript renumbering rewrites only the digit core of a run (`re.fullmatch(r'(\D*)(\d(?:.*\d)?)(\D*)', text, re.S)`) as the mapped, re-collapsed list via `renumber_bracket_group`, reattaching the run's own prefix/suffix, so Word's rsid-split runs keep their separators: `3,` + `4` → `10,` + `11` (never `10` + `11` = 1011), `3-` + `5` → `10-` + `12`, `3 ` → `10 `.
...
Also pin Word's rsid-split runs (each superscript piece is a run of its own): in `tests/test_bracket_helpers.py::TestApplyRenumberingSuperscript`, `"3," + "4"` with `{3: 10, 4: 11}` → `"10," + "11"`, `"3-" + "5"` and `"3" + "-5"` with `{3: 10, 4: 11, 5: 12}` → `"10-" + "12"` / `"10" + "-12"`, and `"3 "` → `"10 "`; in `TestExpandBracketNumbers`, `expand_bracket_numbers("3-")` is `[]` but `[3]` with `lenient=True`; and in `tests/test_field_aware_scanners.py` a `"3-" + "5"` variant of `_cited_doc` where the scanner reports `[[3], [5]]`, the parser `[3, 5]` and `apply_renumbering` writes `10-12`.
...
1. Create `src/pipeline/citation_numbers.py` containing `expand_bracket_numbers` and `format_bracket_numbers` (moved verbatim from `renumber_apply.py:15-64`). In `renumber_apply.py` replace the definitions with `from .citation_numbers import expand_bracket_numbers, format_bracket_numbers  # noqa: F401`. Give `expand_bracket_numbers` a keyword `lenient: bool = False`: a token that is neither a number nor a well-formed range is dropped by default (bracket groups matched by `BRACKET_CITE_PATTERN` are always well-formed), but with `lenient` it contributes the numbers written in it (`3-` → `[3]`, `1, 3-` → `[1, 3]`). Word splits a superscript run at revision (rsid) boundaries, so the scanner and the parser pass `lenient=True`; `renumber_bracket_group` stays strict.
2. `docx_io.find_superscript_citation_runs(self, skip_fields: bool = True)`: inside the loop, `if skip_fields and self.fields.in_field(run): char_offset += len(run.text); continue`; compute `nums = expand_bracket_numbers(run.text, lenient=True)` (import inside the method to avoid a circular import) so a split `3-` run is reported as `[3]`, not dropped.
3. `existing_citation_parser._scan_in_text_citations(self, paragraphs, valid_numbers)`: keep the signature; for each paragraph get `spans = self.handler.fields.result_spans(para)` and `def _in_field_result(offset): return any(s <= offset < e for s, e in spans)`; for superscript runs skip `self.handler.fields.in_field(run)`; replace the inner `re.finditer(r'\d+', run.text)` loop with iterating `expand_bracket_numbers(run.text, lenient=True)` (all numbers share the run's start offset); for bracket matches also skip when `_in_field_result(m.start())`. Delete `_expand_citation_range`; use `expand_bracket_numbers` from `citation_numbers`.
4. `apply_renumbering`: superscript loop — `cite_runs = handler.find_superscript_citation_runs()` already skips fields; replace the `re.sub(r'\d+', ...)` with a rewrite of the run's digit core only: `m = re.fullmatch(r'(\D*)(\d(?:.*\d)?)(\D*)', text, re.S)` (skip the run when `m` is None: separators only), `prefix, core, suffix = m.groups()`, `new_text = prefix + renumber_bracket_group(core, renumber_map) + suffix` (handles ranges and dedup; keep the `CITATION_SHAPE_PATTERN` gate). Never reformat the whole run text: Word splits a superscript citation into several runs at rsid boundaries, and `renumber_bracket_group("3,")` returns `"10"`, which turns `"3," + "4"` into `"10" + "11"` (reads as 1011) and drops the trailing space of `"3 "`; with the prefix/suffix reattached the pieces become `"10," + "11"`, `"10-" + "12"` and `"10 "`. Bracket loop — get `spans = handler.fields.result_spans(para)` and skip matches whose `match.start()` falls in a span. Note `format_bracket_numbers` emits `", "` separators; for superscript runs re-join with `","` (no space) to preserve today's superscript shape: after `renumber_bracket_group`, `.replace(", ", ",")` on the mapped core unless the run's own text already used `", "`.
