...
def test_several_papers_on_a_multi_marker_sentence_fill_that_slot(tab):
    ...
    widgets = ref_widgets(tab)
    assert [w.citation.title for w in widgets] == ["Paper 4", "Paper 9", "Paper 8"]
    assert [w.slot for w in widgets] == [0, 1, 1]            # both picks belong to marker 2

    widgets[0].accept_btn.click()
    assert [c.title for c in ev.selected] == ["Paper 4", "Paper 9", "Paper 8"]
