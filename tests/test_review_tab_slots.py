"""Review tab: slot-aware editing of multi-marker sentences (offscreen Qt).

The reference cards are the source of truth while the user decides;
``evidence.selected`` and ``slot_sizes`` are rebuilt from them once every
card has a decision, one block of references per marker.
"""

import os

import pytest

pytestmark = pytest.mark.skipif(os.environ.get("QT_QPA_PLATFORM") != "offscreen", reason="needs offscreen Qt")

from PySide6.QtWidgets import QLabel  # noqa: E402

from src.models.citation import Author, CitationCandidate  # noqa: E402
from src.models.evidence import EvidenceRecord, ReviewDecision  # noqa: E402
from src.models.project import ProjectState  # noqa: E402
from src.models.sentence import SentenceRecord  # noqa: E402
from src.pipeline.export_slots import collect_marker_slots, ACTION_CITE, ACTION_LEAVE, ACTION_UNRESOLVED  # noqa: E402
from src.utils.markers import find_markers  # noqa: E402


def cand(pmid, title=None):
    return CitationCandidate(pmid=pmid, title=title or f"Paper {pmid}", year=2020,
                             authors=[Author(last_name="A")], composite_score=50.0)


def make_project(text, selected, slot_sizes):
    s = SentenceRecord(id="S001", paragraph_index=0, sentence_index=0, raw_text=text, clean_text=text)
    s.markers = find_markers(text)
    s.marker_types = [m.kind for m in s.markers]
    s.marker_type = s.markers[0].kind
    s.marker_count = len(s.markers)
    p = ProjectState(sentences=[s])
    p.settings.reference_library_path = ""          # no SQLite library in tests
    p.evidence_map[s.id] = EvidenceRecord(sentence_id=s.id, selected=selected, slot_sizes=slot_sizes)
    return p


@pytest.fixture(scope="module")
def app():
    from PySide6.QtWidgets import QApplication
    return QApplication.instance() or QApplication([])


@pytest.fixture
def review_tab(app):
    from src.gui.review_tab import ReviewTab
    tab = ReviewTab()
    yield tab
    tab.close()


def ref_widgets(tab):
    from src.gui.review_tab import SingleRefWidget
    return [w for w in tab._per_ref_widgets if isinstance(w, SingleRefWidget)]


def info_labels(tab):
    return [w.text() for w in tab._per_ref_widgets if isinstance(w, QLabel)]


def test_placeholder_card_for_each_empty_slot(review_tab):
    project = make_project("A (REF) and B (PMID: 1) and C (REF).", [cand("9")], [0, 1, 0])
    review_tab.load_project(project)
    review_tab.sentence_list.setCurrentRow(0)

    ev = project.evidence_map["S001"]
    widgets = ref_widgets(review_tab)
    assert len(widgets) == 3
    assert [w.slot for w in widgets] == [0, 1, 2]
    assert [w.slot_label for w in widgets] == ["(REF)", "(PMID: 1)", "(REF)"]
    assert widgets[1].citation.pmid == "9" and widgets[1].accept_btn.isEnabled()
    assert not widgets[0].accept_btn.isEnabled() and not widgets[2].accept_btn.isEnabled()
    # Placeholders live in the cards only; the evidence keeps its real layout
    assert [c.pmid for c in ev.selected] == ["9"] and ev.slot_sizes == [0, 1, 0]
    slots = collect_marker_slots(project)[0]
    assert [x.citations == [] for x in slots] == [True, False, True]


def test_remove_and_replace_rebuild_the_slots(review_tab):
    project = make_project("A (REF) and B (REFS).", [cand("1"), cand("2"), cand("3")], [1, 2])
    review_tab.load_project(project)
    review_tab.sentence_list.setCurrentRow(0)
    ev = project.evidence_map["S001"]

    widgets = ref_widgets(review_tab)
    assert [w.slot for w in widgets] == [0, 1, 1]
    widgets[1]._on_remove()          # remove paper 2 (first of the REFS slot)
    assert [c.pmid for c in ev.selected] == ["1", "2", "3"]   # untouched until every card is decided

    # Replace the remaining REFS card: the replacement lands on that card, not the removed one
    review_tab._replace_ref_index = 2
    review_tab._replace_ref_sentence_id = "S001"
    review_tab._on_single_ref_fetched(cand("77", "Replacement"))
    assert widgets[2].citation.pmid == "77" and widgets[2]._accepted
    assert widgets[1].citation.pmid == "2"       # removed card untouched
    assert ev.review_decision == ReviewDecision.PENDING

    widgets[0]._on_accept()
    assert [c.pmid for c in ev.selected] == ["1", "77"]
    assert ev.slot_sizes == [1, 1]
    assert ev.review_decision == ReviewDecision.MODIFIED
    assert ev.candidates[0].pmid == "77"
    slots = collect_marker_slots(project)[0]
    assert [x.action for x in slots] == [ACTION_CITE, ACTION_CITE]
    assert [c.pmid for c in slots[1].citations] == ["77"]


def test_skipped_multi_ref_sentence_can_be_accepted_again(review_tab):
    project = make_project("A (REF) and B (PMID: 2).", [cand("1"), cand("2")], [1, 1])
    review_tab.load_project(project)
    review_tab.sentence_list.setCurrentRow(0)
    ev = project.evidence_map["S001"]

    review_tab._on_skip()
    assert ev.review_decision == ReviewDecision.SKIPPED and ev.is_resolved
    assert [x.action for x in collect_marker_slots(project)[0]] == [ACTION_LEAVE, ACTION_LEAVE]
    review_tab.sentence_list.setCurrentRow(0)
    review_tab._on_sentence_selected(review_tab.sentence_list.item(0), None)
    assert any("Left unchanged" in t for t in info_labels(review_tab))
    review_tab._on_accept_all_refs()
    assert ev.review_decision == ReviewDecision.ACCEPTED
    assert ev.slot_sizes == [1, 1]
    assert [x.action for x in collect_marker_slots(project)[0]] == [ACTION_CITE, ACTION_CITE]


def test_empty_multi_marker_sentence_uses_per_ref_mode(review_tab):
    project = make_project("A (REF) and B (REF).", [], [0, 0])
    review_tab.load_project(project)
    review_tab.sentence_list.setCurrentRow(0)
    widgets = ref_widgets(review_tab)
    assert len(widgets) == 2 and all(not w.accept_btn.isEnabled() for w in widgets)
    assert [w.slot for w in widgets] == [0, 1]
    ev = project.evidence_map["S001"]
    assert ev.slot_sizes == [0, 0] and ev.selected == []
    # Accept All must not "keep" placeholders
    review_tab._on_accept_all_refs()
    assert ev.review_decision == ReviewDecision.PENDING


def test_replace_keeps_slot_layout(review_tab):
    project = make_project("A (REF) and B (PMID: 1).", [cand("1"), cand("2")], [1, 1])
    review_tab.load_project(project)
    review_tab.sentence_list.setCurrentRow(0)
    ev = project.evidence_map["S001"]

    review_tab._replace_ref_index = 1
    review_tab._replace_ref_sentence_id = "S001"
    review_tab._on_single_ref_fetched(cand("77", "Replacement"))
    ref_widgets(review_tab)[0]._on_accept()
    assert [c.pmid for c in ev.selected] == ["1", "77"]
    assert ev.slot_sizes == [1, 1]
    assert ev.candidates[0].pmid == "77"


def test_several_chat_picks_join_the_same_slot(review_tab):
    project = make_project("A (REF) and B (PMID: 1).", [cand("1"), cand("2")], [1, 1])
    review_tab.load_project(project)
    review_tab.sentence_list.setCurrentRow(0)
    ev = project.evidence_map["S001"]

    review_tab._on_single_ref_chat(1)
    review_tab._on_chat_citation_selected([cand("8"), cand("9")])
    widgets = ref_widgets(review_tab)
    assert [w.citation.pmid for w in widgets] == ["1", "8", "9"]
    assert [w.slot for w in widgets] == [0, 1, 1]
    widgets[0]._on_accept()
    assert [c.pmid for c in ev.selected] == ["1", "8", "9"]
    assert ev.slot_sizes == [1, 2]
    assert [c.pmid for c in collect_marker_slots(project)[0][1].citations] == ["8", "9"]


def test_whole_sentence_chat_pick_fills_the_empty_slot(review_tab):
    project = make_project("A (REF) and B (REF).", [cand("1")], [1, 0])
    review_tab.load_project(project)
    review_tab.sentence_list.setCurrentRow(0)
    ev = project.evidence_map["S001"]

    review_tab._on_modify()
    review_tab._on_chat_citation_selected([cand("5")])
    assert [c.pmid for c in ev.selected] == ["1", "5"]
    assert ev.slot_sizes == [1, 1]
    assert ev.review_decision == ReviewDecision.MODIFIED


def test_leave_unchanged_marks_sentence_skipped(review_tab):
    project = make_project("Claim (Smith 2020).", [], [0])
    review_tab.load_project(project)
    review_tab.sentence_list.setCurrentRow(0)
    assert review_tab.skip_btn.isEnabled() and not review_tab.skip_btn.isHidden()
    assert any("Author-suggested (Smith 2020): Smith 2020" in t for t in info_labels(review_tab))
    (placeholder,) = ref_widgets(review_tab)
    assert not placeholder.accept_btn.isEnabled() and placeholder.replace_btn.isEnabled()

    review_tab._on_skip()
    ev = project.evidence_map["S001"]
    assert ev.review_decision == ReviewDecision.SKIPPED and ev.is_resolved
    assert project.all_resolved
    assert review_tab.export_btn.isEnabled()
    slots = collect_marker_slots(project)[0]
    assert slots[0].action == ACTION_LEAVE


def test_suggested_details_and_highlight(review_tab):
    project = make_project("Spines grow (PMID: 1) here.", [cand("1")], [1])
    ev = project.evidence_map["S001"]
    ev.selected[0].score_rationale = "Author-suggested (PMID 1): directly supports"
    ev.confidence_rationale = "All good"
    review_tab.load_project(project)
    review_tab.sentence_list.setCurrentRow(0)

    texts = info_labels(review_tab)
    assert any("Author-suggested (PMID: 1): PMID 1" in t for t in texts)
    assert any("AI assessment: All good" in t for t in texts)
    html_text = review_tab.sentence_text.toHtml()
    assert "(PMID: 1)" in html_text
    item = review_tab.sentence_list.item(0)
    assert "(PMID: 1)" in item.text()

    # Filter: author-suggested only keeps this row visible
    review_tab.filter_combo.setCurrentIndex(4)
    assert not item.isHidden()


def test_rejected_and_pending_ref_markers_still_get_question_mark(review_tab):
    project = make_project("Claim (REF).", [], [0])
    review_tab.load_project(project)
    review_tab.sentence_list.setCurrentRow(0)
    ev = project.evidence_map["S001"]
    assert collect_marker_slots(project)[0][0].action == ACTION_UNRESOLVED
    ev.review_decision = ReviewDecision.REJECTED
    assert not ev.is_resolved
    assert collect_marker_slots(project)[0][0].action == ACTION_UNRESOLVED
